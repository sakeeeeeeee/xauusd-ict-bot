...
st.markdown(
    "Dashboard ini menampilkan riwayat trade dari database SQLite secara real-time."
)
...
wins = len(
    resolved_df[resolved_df["result"].str.contains("WIN|TP1 \+ BEP", regex=True)]
)
...
if not resolved_df.empty:
    ...
    weekly_stats = (
        resolved_df.groupby("week")
        .apply(
            lambda x: pd.Series(
                {
                    "total": len(x),
                    "wins": len(
                        x[x["result"].str.contains("WIN|TP1 \+ BEP", regex=True)]
                    ),
                }
            )
        )
...
selected_side = st.sidebar.multiselect(
    "Filter by Side:", options=df["side"].unique(), default=df["side"].unique()
)

selected_result = st.sidebar.multiselect(
    "Filter by Result:", options=df["result"].unique(), default=df["result"].unique()
)

# Terapkan filter
filtered_df = df[
    (df["side"].isin(selected_side)) & (df["result"].isin(selected_result))
]
...
st.dataframe(
    filtered_df[columns_to_show].style.format(
        {
            "entry": "{:.2f}",
            "sl": "{:.2f}",
            "tp1": "{:.2f}",
